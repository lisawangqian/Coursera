...
def spawn_ball(direction):
    ...
    ball_pos = [WIDTH / 2, HEIGHT / 2]
    ball_vel[0] = -random.randrange(120, 240)/180
    ball_vel[1] = -random.randrange(60, 180)/120
...
def draw(canvas):
    ...
    if (ball_pos[0] >= WIDTH - BALL_RADIUS - PAD_WIDTH) or (ball_pos[0] <= BALL_RADIUS + PAD_WIDTH):
        if (paddle1_pos - HALF_PAD_HEIGHT <= ball_pos[1] <= paddle1_pos + HALF_PAD_HEIGHT) and (ball_vel[0] < 0):
            ball_vel[0] = -(1 + 0.1) * ball_vel[0]
            ball_vel[1] = (1 + 0.1) * ball_vel[1]
    ...
    if (paddle2_pos <= HEIGHT - HALF_PAD_HEIGHT and paddle2_vel > 0) or (paddle2_pos >=  HALF_PAD_HEIGHT and paddle2_vel < 0) :
    ...
    canvas.draw_polygon([[WIDTH, paddle2_pos-HALF_PAD_HEIGHT], [WIDTH - PAD_WIDTH, paddle2_pos-HALF_PAD_HEIGHT], [WIDTH - PAD_WIDTH, paddle2_pos + HALF_PAD_HEIGHT], [WIDTH, paddle2_pos + HALF_PAD_HEIGHT]],2, "White", "White")
    
    
    # draw scores
    canvas.draw_text(str(score1), [(WIDTH / 2) - (WIDTH / 4)- 25, 50], 50, "Yellow")
    canvas.draw_text(str(score2), [(WIDTH / 2) + (WIDTH / 4), 50], 50, "Yellow")    
