...
def draw(canvas):
    ...
    # draw and updateship and sprites
    my_ship.draw(canvas)
...
def rock_spawner():
    ...
    if started and len(rock_group) < 12 and dist(rock_pos, my_ship.get_position()) > 90:
...
def group_collide(group_set, other_object):
    ...
    group_set.difference_update(remove)
    if remove:
        return True
    else:
        return False
...
def group_group_collide(group_set1, group_set2):
    group_set1_copy = group_set1
    l = 0
    for element in group_set1:
        if group_collide(group_set2, element):
           l += 1
           group_set1_copy.discard(element)
    
    group_set1 = group_set1_copy        
